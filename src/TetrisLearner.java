...
public interface TetrisLearner {
	...
	/**
	 * Learn how to solve a Tetris during a certain duration. It tries man many
	 * different ways and improves himself
	 * 
	 * @param solver
	 *            The solver used to resolve Tetris
	 * @param duration
	 *            represent the limit/time/iterations that have the learner
	 *            before finishing
	 * @param maxLine
	 *            represent the maximum number of line to remove in the tetris
	 *            before considering it infinity
	 * @param averageGamePlayed
	 *            The number of games taken to do an average
	 * @param startingWeights
	 *            the starting weights used for the very first try of the
	 *            learner
	 ...
	 * @param maxLine
	 *            Maximum score we can get, (avoid infinite loop if a AI cannot
	 *            die)
	 * @param n
	...
	public static int solveAvg(PlayerSkeleton.TetrisSolver solver, float[] weights, int maxLine, int n, int lastValue) {
		...
		try (Writer writer = new BufferedWriter(new FileWriter("count.csv", true))) {
			...
			for (int i = 0; i < n; i += threads) {
				...
				int sumOfLinesCompleted = IntStream.range(0, threads).parallel()
						.map(x -> runStateTillCompletion(solver, new State(), maxLine, weights)).sum();
	...
	/**
	 * Run a given state, play till it reaches game over, or has cleared maxLine
	 * number of lines
	 *
	 * @param solver
	 *            The AI/player that need to play
